...
const TOOLS = [
  {
    ...
    title: "위치·도시 검색 날씨 예보 | WebToolBay",
    description:
      "Open-Meteo 기반 현재·24시간·7일 예보와 체감온도·강수 확률을 확인하는 WebToolBay 날씨 전용 페이지입니다.",
    guideTitle: "날씨 예보 전용 화면 이용 안내",
    intro:
      "이 URL은 홈의 날씨 카드가 아니라, 위치 권한·도시 검색·°C/°F·시간별·7일 예보를 한곳에 모은 전용 화면입니다. 기상청 특보·재난 문자를 대체하지 않으며, 일상 계획용 참고 예보로 쓰세요.",
    usage: [
  ...
  },
  {
    ...
    title: "브라우저 이미지 자르기·회전 편집기 | WebToolBay",
    description:
      "업로드 없이 브라우저에서 이미지를 자르고 회전·반전·리사이즈하는 WebToolBay 전용 편집기입니다.",
    guideTitle: "이미지 자르기·회전 편집기 이용 안내",
    intro:
      "WebToolBay 이미지 편집기는 SNS 썸네일, 증명 사진 비율, 기울어진 촬영본처럼 “조금만 다듬으면 되는” 작업을 위한 전용 화면입니다. 홈 목록의 한 줄 소개와 달리, 이 URL에서만 자르기 박스·비율 고정·저장 형식 옵션을 바로 사용할 수 있습니다.",
    usage: [
    ...
    faq: [
      ...
      {
        q: "홈페이지와 같은 기능인가요?",
        a: "홈은 도구 모음 안내만 합니다. 실제 자르기·회전 UI와 저장은 이 /tools/editor.html 전용 페이지에서만 동작합니다."
      },
      {
        q: "HEIC·움직이는 GIF도 되나요?",
        a: "브라우저가 읽지 못하는 HEIC는 JPG 등으로 보낸 뒤 올리세요. 애니메이션 GIF·움직이는 WebP는 첫 장면만 편집됩니다."
      }
    ],
    guides: [
      { href: "guides/editor.html", label: "이미지 자르기·회전 가이드" },
  ...
  },
  {
    ...
    title: "발표 대본 예상 시간 계산기 | WebToolBay",
    description:
      "대본을 붙여 넣고 말하기 속도로 예상 발표 시간과 분량 과부족을 계산하는 WebToolBay 전용 도구입니다.",
    guideTitle: "발표 시간 계산기 이용 안내",
    intro:
      "홈의 한 줄 소개와 달리, 이 화면에서는 대본·속도 프리셋·목표 시간을 넣어 예상 소요를 숫자로 확인합니다. 수업 발표, 면접, 회의 브리핑처럼 “이 원고를 읽으면 몇 분이나 걸릴지”를 가늠할 때 쓰세요.",
    usage: [
  ...
  },
  {
    ...
    title: "D−/D+ 디데이 날짜 계산기 | WebToolBay",
    description:
      "기준일과 목표일로 남은 날(D−)·당일(D-Day)·지난 날(D+)을 계산하는 WebToolBay 디데이 전용 페이지입니다.",
    guideTitle: "디데이 날짜 계산기 이용 안내",
    intro:
      "홈 카드 설명이 아니라, 기준일·목표일·제목을 입력해 D− / D-Day / D+를 구분하는 계산 화면입니다. 수능·자격증·결혼식·마감처럼 “그날까지 며칠인지”를 숫자로 볼 때 쓰며, 브라우저에서만 계산합니다.",
    usage: [
...
const DETAILS = {
  weather: [
    ...
    "이 페이지는 홈의 ‘날씨 카드’와 달리, Open-Meteo 값을 시간대·일별로 펼쳐 보여주는 전용 화면입니다. VPN·회사망을 켠 상태면 위치가 어긋날 수 있으니, 그럴 때는 도시 검색으로 목적지를 직접 고른 뒤 새로고침하세요.",
    "강수 확률 숫자만으로 우산을 정하기 어렵다면 강수 확률 해석 가이드를 함께 보고, 공식 특보가 필요할 때는 기상청 안내를 우선하세요."
  ],
  calendar: [
  ...
  editor: [
    "이 화면은 홈에 있는 소개 카드가 아니라, Canvas로 자르기·회전·반전·리사이즈를 실제로 수행하는 전용 편집기입니다. 원본 파일은 서버로 올라가지 않고 브라우저 메모리에서만 다루며, 저장 버튼을 눌러야 새 파일이 내려받아집니다.",
    "자르기는 ‘무엇을 남길지’를 먼저 정한 뒤에 하는 편이 덜 헤맵니다. SNS·유튜브용이면 1:1·16:9·9:16처럼 비율을 먼저 고정하고, 인물이면 얼굴 여백, 문서·OCR용이면 글자 가장자리가 잘리지 않게 프레임을 맞추세요.",
    "회전·반전은 OCR이나 업로드 전에 하면 인식률과 미리보기 품질이 좋아지는 경우가 많습니다. 기울어진 영수증·칠판 사진은 수평을 맞춘 뒤 텍스트 추출로 넘기는 순서가 실수 없이 가장 짧았습니다.",
    "과도하게 키우면 선명도가 떨어집니다. 키우기보다 원본에 가깝게 두고, 용량·확장자가 필요하면 저장 후 이미지 형식 변환으로 이어가세요. Safari 계열은 WebP 저장이 막힐 수 있어 PNG·JPG를 쓰는 편이 안전합니다.",
    "전문 포토샵의 레이어·브러시는 없습니다. ‘조금만 다듬으면 되는’ 썸네일·증명사진·기울기 보정에 맞춰 두었고, 작업 중에는 한 단계 되돌리기·원본 초기화로 실수를 바로잡을 수 있습니다."
  ],
  picker: [
  ...
  speech: [
    "이 페이지는 홈의 짧은 소개와 달리, 대본 글자·단어 수와 말하기 속도 슬라이더로 예상 시간을 계산하는 전용 화면입니다. 기본값은 한국어 분당 약 300자, 영어 분당 약 130단어를 기준으로 둡니다.",
    "목표 시간보다 30초~1분 짧게 나오는 분량으로 맞추면, 실제 발표에서 숨 고르기·슬라이드 전환 여유가 생깁니다. 반대로 계산이 목표보다 길면 예시·부연을 먼저 줄이는 편이 속도만 올리는 것보다 자연스럽습니다.",
    "한영이 섞인 원고는 구간별로 나눠 계산해 보면 어느 부분이 시간을 잡아먹는지 보이기 쉽습니다. 리허설 후 본인 보통 속도를 메모해 두면 다음 발표 준비가 빨라집니다.",
    "입력한 대본은 서버로 보내지 않습니다. 침묵·데모·질의응답 시간은 계산에 포함되지 않으니, 제한 시간 발표에서는 별도 여유를 남기세요."
  ],
  dday: [
    "홈 카드의 한 줄 설명과 달리, 이 화면은 기준일·목표일을 넣어 D− / D-Day / D+를 구분하는 계산기입니다. 초일불산입(날짜 차이) 방식이라, 만난 날을 1일로 세는 100일·백일 기념일과 숫자가 다를 수 있습니다.",
    "디데이는 ‘압박용 숫자’보다 ‘계획 단위를 나누는 기준’으로 쓸 때 효과가 큽니다. 예를 들어 D−30에 초안, D−7에 최종본처럼 마일스톤을 정하면 막판 몰아치기를 줄일 수 있습니다.",
    "기준일을 오늘이 아닌 날짜로 바꾸면, 휴가·출장 전에 미리 남은 날을 시뮬레이션할 수 있습니다. 음력 기념일은 양력·음력 변환으로 양력 날짜를 구한 뒤 목표일에 넣어야 달력과 맞습니다.",
    "영업일·공휴일만 세는 계산과는 다릅니다. 날짜는 브라우저에서만 계산되며 서버로 보내지 않습니다."
  ],
  noise: [
